...
namespace cs31
{
    ...
    BeatThat::GAMEOUTCOME  BeatThat::determineGameOutcome( ) const
    {
        ...
        GAMEOUTCOME result;
        if ((MAX_NUMBER_OF_TURNS == mTurnCount) && (mHuman.getRoundsWon() > mComputer.getRoundsWon()))
            result = GAMEOUTCOME::HUMANWONGAME;
        else if ((MAX_NUMBER_OF_TURNS == mTurnCount) && (mComputer.getRoundsWon() > mHuman.getRoundsWon()))
            result = GAMEOUTCOME::COMPUTERWONGAME;
        else if ((MAX_NUMBER_OF_TURNS == mTurnCount) && (mHuman.getRoundsWon() == mComputer.getRoundsWon()))
